...
import { loadDashboardLatestSalesMetrics } from "@/app/api/_lib/dashboard-analytics";
...
export async function GET(request: Request) {
  ...
  try {
    const data = await loadDashboardLatestSalesMetrics(adminClient, {
      year,
      month,
      limit,
    });
    ...
    return NextResponse.json(
      {
        message: error instanceof Error ? error.message : "No se pudieron cargar las ultimas ventas.",
      },
      { status: 500 },
    );
  }
