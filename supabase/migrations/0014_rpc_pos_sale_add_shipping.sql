-- Drop the old function signature so we can recreate with additional params
drop function if exists public.confirm_pos_sale(uuid, uuid, jsonb, text, uuid, text, integer);
...
grant execute on function public.confirm_pos_sale(uuid, uuid, jsonb, text, uuid, text, integer, jsonb) to authenticated;
